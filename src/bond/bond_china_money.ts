...
/**
 * 收盘收益率曲线映射数据（债券类型代码映射）
 * https://www.chinamoney.com.cn/chinese/bkcurvclosedyhis/?bondType=CYCC000&reference=1
 */
export async function bond_china_close_return_map_v2(): Promise<DataFrame> {
  const url =
 ...
 */
export async function bond_china_close_return_v2(
  symbol: string = '国债',
  period: string = '1',
  ...
  try {
    // 先获取债券类型映射
    const mapDf = await bond_china_close_return_map_v2();
    ...
    const symbolRow = mapDf.data.find(
      (row) => row[cnLabelIndex] === symbol
    );
    if (!symbolRow) {
      return createDataFrame([], []);
    }
